...
function rlpDecodeItem(data) {
    ...
    if (data[0] <= 0x7f) {
        ...
        if (lengthBytesLength > data.length - 1)
            throw new Error(`Encoded length of data length (${lengthBytesLength}) is larger than the remaining data (${data.length - 1})`);
        ...
        if (length > data.length - 1 - lengthBytesLength)
            throw new Error(`Encoded data length (${length}) is larger than the remaining data (${data.length - 1 - lengthBytesLength})`);
        ...
        while (offset !== length + 1) {
            ...
            offset += consumed;
            if (offset > length + 1)
                throw new Error(`Encoded array length (${length}) doesn't align with the sum of the lengths of the encoded elements (${offset})`);
        }
        ...
        const length = decodeLength(data, 1, lengthBytesLength);
        if (length > data.length - 1 - lengthBytesLength)
            throw new Error(`Encoded array length (${length}) is larger than the remaining data (${data.length - 1 - lengthBytesLength})`);
...
function decodeLength(data, offset, lengthBytesLength) {
